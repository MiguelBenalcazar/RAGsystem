...
from semantic_chunkers.splitter.sentence import RegexSplitter
from semantic_chunkers.utils.text import token_length
...
class StatisticalChunker():
    ...
    def _chunk(self, splits: List[Any], batch_size: int = 64, enforce_max_tokens: bool = False) -> List[Chunk]:
        ...
        if enforce_max_tokens:
            new_splits = []
            for split in splits:
                token_count = token_length(split)
    ...
    def _split(self, doc:str)-> str:
        split = self.splitter(doc)
        return split
    ...
    def __call__(self, docs: List[str], batch_size: int = 64) -> List[List[Chunk]]:
        ...
        for doc in docs:
            ...
            if isinstance(doc, str):
                splits = self._split(doc) # clean data
    ...
    def _find_optimal_threshold(self, docs: List[str], similarity_scores: List[float]):
        token_counts = [token_length(doc) for doc in docs]
    ...
    def _split_documents(
        ...
        split_token_count = sum(
            [token_length(doc) for split in chunks for doc in split.splits]
        )
    ...
    def plot_similarity_scores(self, similarities: List[float], split_indices: List[int], chunks: list[Chunk])->None:
    ...
    def plot_sentence_similarity_scores(self, docs: List[str], threshold: float, window_size: int)->None:
        try:
            from matplotlib import pyplot as plt
        except ImportError:
